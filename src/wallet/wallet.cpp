...
// oblivion: optional setting to unlock wallet for block staking only;
...
//    return n ? n : nTimeReceived;
    ...
    // oblivion: we still have the timestamp, so use it to avoid confusion
    return tx->nTime;
}
...
// oblivion: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            if (nSpendTime > 0 && pcoin->tx->nTime > nSpendTime)
                continue;  // oblivion: timestamp must not exceed spend time
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    txNew.nTime = wtxNew.tx->nTime;  // oblivion: set time
...
//                }
                ...
                // oblivion: sub-cent change is moved to fee
...
// oblivion: create coin stake transaction
